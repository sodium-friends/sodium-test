...
module.exports = function (sodium) {
  test('crypto_box_keypair generate key-pair', function (t) {
    ...
    const pubKeyCopy = Buffer.alloc(sodium.crypto_box_PUBLICKEYBYTES)
    pubKey.copy(pubKeyCopy)

    const secretCopy = Buffer.alloc(sodium.crypto_box_SECRETKEYBYTES)
    secret.copy(secretCopy)

    sodium.crypto_box_keypair(pubKey, secret)
    t.not(pubKey, pubKeyCopy)
    t.not(secret, secretCopy)
  })
  ...
  test('crypto_box2', t => {
    ...
    const msize = 7 + Math.floor(Math.random() * 1000)
    const m = new Uint8Array(msize)
    const m2 = new Uint8Array(msize)
    const c = new Uint8Array(sodium.crypto_box_MACBYTES + msize)

    sodium.crypto_box_keypair(alicepk, alicesk)
    sodium.crypto_box_keypair(bobpk, bobsk)

    const mlen = Math.floor(Math.random() * msize) + 1
